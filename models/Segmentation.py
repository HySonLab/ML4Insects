...
def get_model(type):
    if type == 'mlp':
        ...
        raise ValueError("Unsupported model type. Param model_type must be one of ['mlp', 'fcn', '2dcnn', 'resnet']")
...
class EPGS:
    ...
    def get_dataloaders(self, r = [0.7, 0.2, 0.1]):
        ...
        dict = datagenerator.generate_input_data(self.dataset_name, verbose = True)
    ...
    def train(self, early_stop = True, patience = 5, min_delta = 0.01, verbose = True):
        assert (self.task == 'train'), f'Task is {self.task}'
        # Initialization 
    ...
    def segment(self, recording_name, verbose = False):
        ...
        if (self._is_model_trained == False) or (self._is_pretrained == False):
